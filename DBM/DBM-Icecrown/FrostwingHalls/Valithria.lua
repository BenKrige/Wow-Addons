...
-- couldn't confirm those values yet, normal delay between 54 and 58?
local initialSuppressorDelay = 30
local suppressorDelay = 56
local timerSuppresserOne	= mod:NewTimer(initialSuppressorDelay, "1st wave of Suppressors", not mod:IsHealer())
local timerSuppresserTwo	= mod:NewTimer(suppressorDelay, "2nd wave of Suppressors", not mod:IsHealer())
local timerSuppresserThree	= mod:NewTimer(suppressorDelay, "3rd wave of Suppressors", not mod:IsHealer())
local timerSuppresserFour	= mod:NewTimer(suppressorDelay, "4th wave of Suppressors", not mod:IsHealer())
...
function mod:OnCombatStart(delay)
	...
	-- TODO: Just make a proper delay function that handles state and delay
	timerSuppresserOne:Start(-delay)

	specWarnSuppresserOne:Schedule(initialSuppressorDelay)
	ttsSuppressersSpawned:Schedule(initialSuppressorDelay)
	timerSuppresserTwo:Schedule(initialSuppressorDelay) -- maybe needs a -1

	local secondSuppressorTime = initialSuppressorDelay+suppressorDelay
	specWarnSuppresserTwo:Schedule(secondSuppressorTime)
	ttsSuppressersSpawned:Schedule(secondSuppressorTime)
	timerSuppresserThree:Schedule(secondSuppressorTime) -- maybe needs a -1

	local thirdSuppressorTime = secondSuppressorTime + suppressorDelay
	specWarnSuppresserThree:Schedule(thirdSuppressorTime)
	ttsSuppressersSpawned:Schedule(thirdSuppressorTime)
	timerSuppresserFour:Schedule(thirdSuppressorTime) -- maybe needs a -1

	local fourthSuppressorTime = thirdSuppressorTime + suppressorDelay
	specWarnSuppresserFour:Schedule(fourthSuppressorTime)
	ttsSuppressersSpawned:Schedule(fourthSuppressorTime)
end
